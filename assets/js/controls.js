...
$('.btn-add-curr-year').on('click', function(e) {
	e.preventDefault();
	var course_code = $(this).data('code');
	$('#form-add-curr-' + course_code).submit();
});

$('.form-add-curr-year').on('submit', function(e) {
	e.preventDefault();
	var course_code = $(this).data('code');
	var formData = new FormData(this);
	$.ajax({
		url: "http://" + window.location.hostname + "/project/index.php/curriculum/add_curriculum/" + course_code,
		type: "post",
		data: formData,
		contentType: false,
	    processData: false,
	    async: false,
	    success: function(dataPass) {
	    	$('.program-curriculum-list').each(function(e) {
